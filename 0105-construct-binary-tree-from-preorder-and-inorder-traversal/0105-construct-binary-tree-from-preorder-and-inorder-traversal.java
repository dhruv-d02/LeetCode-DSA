...
class Solution {
    ...
    private TreeNode build(int[] preorder, int preStart, int preEnd, int[] inorder, int inStart, int inEnd, HashMap<Integer, Integer> inMap) {
        ...
        root.left = build(preorder, preStart + 1, preStart + numsLeft, inorder, inStart, rootPos - 1, inMap);
        root.right = build(preorder, preStart + numsLeft + 1, preEnd, inorder, rootPos + 1, inEnd, inMap);
        return root;
    }
}